...
if __name__ == '__main__':
    ...
    LVopt = LVopt_Type.LV_Optimization()
    ...
    for i, RecoveryStrategy in enumerate(RecoveryStrategyVec):
        ...
        ax_mass.plot(Solution['t1_vec'], Solution['x1'][4,:]/1000, color=col, label = f'Payload = {mass:.6}[Kg]')
        ...
        tf = 0
        while True:
            x_parking_scaled = LVopt.eci.dynamics_kp1(x_parking_scaled, 0, 1.0/LVopt.eci.scaleT)
        ...
        while True:
            x_final_scaled = LVopt.eci.dynamics_kp1(x_final_scaled, 0, 1.0/LVopt.eci.scaleT)
        ...
        x = LVopt.eci.R0 * np.sin(phi) * np.cos(theta)
        y = LVopt.eci.R0 * np.sin(phi) * np.sin(theta)
        z = LVopt.eci.R0 * np.cos(phi)

        # Create equatorial plane (z=0)
        theta_plane = np.linspace(0, 2*np.pi, 100)
        r_plane = np.linspace(0, LVopt.eci.R0*1.1, 25)
        ...
        #plot trajectory
        # ax_3d.plot(Solution['x3'][0,:], Solution['x3'][1,:], Solution['x3'][2,:], 'r')
        ax_3d.plot(Solution['x2'][0,:], Solution['x2'][1,:], Solution['x2'][2,:], color=col)
        ax_3d.plot(Solution['x3'][0,:], Solution['x3'][1,:], Solution['x3'][2,:], color=col)
        ax_3d.plot(x_parking[:,0], x_parking[:,1], x_parking[:,2], color=col, label=f'Parking Orbit (apogee={np.round((np.max(r_parking) - LVopt.eci.R0)/1000)}, perigee={np.round((np.min(r_parking) - LVopt.eci.R0)/1000)})')
        ax_3d.plot(x_final[:,0], x_final[:,1], x_final[:,2], '--', color=col, label=f'Final Orbit (apogee={np.round((np.max(final_apogee) - LVopt.eci.R0)/1000)}, perigee={np.round((np.min(final_perigee) - LVopt.eci.R0)/1000)})')
        ax_3d.legend()

        # ECI axes
        max_range = LVopt.eci.R0 * 2
