    ...
    for iOrbit, Target_Orbit in enumerate(Target_Orbits):
        ...
        for iRecover, RecoveryStrategy in enumerate(RecoveryStrategyVec):
            ...
            for iVal, DispesrionValue in enumerate(DispesrionVec):
                ...
                # DispesrionFactors.FirstStageThrust = 0.95
                # DispesrionFactors.SecondStageThrust = 0.95            
                ...
                print(f"Recovery Strategy: {RecoveryStrategy}, DispesrionParam: {DispesrionParam}, DispesrionValue: {DispesrionValue:.4}, Payload Mass: {Solution['payload_mass']:.1f} [kg]")
                if iVal == DispesrionVec.shape[0]-1:
                    print(Target_Orbit['Name']+'_'+RecoveryStrategy+'_'+DispesrionParam+'=['+', '.join(f'{x:.3f}' for x in Results) + ']')
