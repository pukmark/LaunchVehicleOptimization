...
if __name__ == '__main__':
    ...
    #                 "i": np.deg2rad(30.0), # inclination [rad]
    #                 }
    ...
    # opti.subject_to(payload_mass <= 25000.0)
    # set the constraints:
    for k in range(N1):
        # set the dynamics
        ...
        opti.subject_to(rocket_Booster.local_to_alt(x1[:,k]) >= 0.0)
    ...
    opti.subject_to(x1[4,N1] >= rocket_Booster.FirstStage_EmptyMass + payload_mass+13000)
    ...
    opti.subject_to(x3[6,0] == x2[6,N2] - rocket_eci.FairingMass)
    for k in range(N3):
        # set the dynamics
    ...
    opti.subject_to(x3[6,N3] >= rocket_eci.SecondStage_EmptyMass + payload_mass + propellent_mass_for_final_dv)
    opti.subject_to(apogee >= Target_Orbit["perigee"])
    opti.subject_to(perigee >= rocket_eci.R0 + 100*1000.0)
    ...
    if RecoveryStrategy != 'None':
        ...
        opti.subject_to(dt4_before_landing >= 20.0)
        opti.subject_to(dt4_landing*N4 >= 5.0)

        # Ballistic phase
        opti.subject_to(x4_before_reentry == rocket_return.dynamics_kp1_M50(x4_0, 0, dt4_ballistic))
        opti.subject_to(rocket_return.local_to_alt(x4_before_reentry) >= 60000.0)
        opti.subject_to(rocket_return.local_to_alt(x4_before_reentry) <= 65000.0)
        ...
        # set the thrust constraints
        opti.subject_to(u4_reentry <= 0.3333)
        ...
        opti.subject_to(rocket_return.local_to_alt(x4_before_landing) >= 1000.0)
        ...
        opti.subject_to(x4_landing[4,N4] >= rocket_Booster.EmptyFirstStageMass) # mass at landing
    ...
    cost += -0.01*payload_mass - 0.001*x3[6,N3]
    cost += 100*((i-Target_Orbit["i"])/(np.deg2rad(0.05)))**2
    # if RecoveryStrategy != 'None':
    #     cost += 0.5*(ca.sumsqr(u4_landing) + ca.sumsqr(u4_reentry) )
        # cost += (dt4_before_landing)*0.001
        # cost += -0.00001*x4_landing[4,N4] # mass at landing
        # cost += (rocket_Booster.EmptyFirstStageMass-x4_landing[4,N4])**2 # mass at landing
    opti.minimize(cost)
...
# calculate the initial guess for second stage after fairing separation
    ...
    iter=0
    alpha3, alpha_factor = np.deg2rad(10.0), 1.0
    while iter < 50:
        for k in range(N3):
            ...
            u3_guess[0:3,k] = (x3_guess[3:6,k] / ca.norm_2(x3_guess[3:6,k])).full().flatten() * rocket_eci.SecondStage_Thrust
            alpha_factor = dt3_guess*k/N3
            alpha3 *= alpha_factor*alpha3
        ...
        if k < N3-1:
            ...
            dt3_guess += 0.1
            iter += 1
        elif apogee > rocket_eci.R0+1000*10**3:
            dt3_guess -= 0.05
            iter += 1
