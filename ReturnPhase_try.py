...
# Phase 4 - Booster Return:
    ...
    opti.subject_to(x4_0[4,0]/(x1_N1_data['x1_N1'][4] - rocket_eci.SecondStage_FullMass - x1_N1_data['payload_mass']) == 1.0)

    # set the time step constraints
    opti.subject_to(dt4_ballistic >= 1.0)
    opti.subject_to(dt4_reentry/10.0 >= 1.0)
    opti.subject_to(dt4_before_landing/10.0 >= 1.0)
    ...
    cost = 0
    cost += -x4_landing[4,N4]/rocket_return.EmptyFirstStageMass
